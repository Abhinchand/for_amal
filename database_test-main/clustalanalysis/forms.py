...
from association.models import MutantProteinDatabase
...
class AnalysisForm(forms.Form):
    ...
    list_mutant_names = forms.CharField(
        widget=forms.HiddenInput(), required=False)
    ...
    def clean_userdataids(self):
        ...
        self.dataids = UserUploadData.objects.filter(
            session_key=self.request.session.session_key).values_list(
            "id", flat=True
        )
        if self.userdata:
    ...
    def clean(self):
        ...
        try:
            self.selected_selected_mutants = ast.literal_eval(
                self.cleaned_data.get("list_mutant_names"))
        except:
            self.mutant_selection = []
        ...
        if self.dataids:
            self.combined_selection += self.dataids
        if self.selected_values:
            self.combined_selection += self.selected_values
        if self.selected_selected_mutants:
            self.mutant_selection += self.selected_selected_mutants

        total = self.mutant_selection + self.combined_selection
        if len(total) <= 3:
            raise forms.ValidationError(
    ...
    def save(self):
        ...
        return (
            self.clustalomega_in_tmp.name,
            self.guidetree_out_tmp.name,
            self.num_lines,
        )
    ...
    def protein_detail_data(self):
        ...
        self.modified = MutantProteinDatabase.objects.filter(
            name__in=self.mutant_selection)
        print("modified", self.modified)
        print("mutant selection", self.mutant_selection)
