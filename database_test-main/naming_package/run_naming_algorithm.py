...
def run_bug(query_data):
    ...
    path = os.listdir(settings.MEDIA_ROOT + '/search_namingalgorithm/')
    # print("path....", path)
    for file in path:
        file_path = path = os.path.join(
            settings.MEDIA_ROOT + '/search_namingalgorithm/') + file
        # print("filepath", file_path)
        if not file:
            pass

        identity_percentage, results = needle_two_sequences(
        ...
        try:
            ...
            print("Unable to convert identity_percentage {}".format(
                identity_percentage))
            identity_percentage = 0.0

        l = file, query_data, identity_percentage
    ...
    # print("identity", percentageidentity)
