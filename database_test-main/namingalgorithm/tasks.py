...
@shared_task
def namingalgorithm_task(filename):
    try:

        # (align, percentageidentity, category, predicted_name,
        #  name,) = run_data.predict_name.run_bug(filename)
        align = run_data.predict_name.run_bug(filename)

        return predicted_name, percentageidentity, align
    except IOError as e:
        print(e)
...
@shared_task
def update_ncbi_accession_status():
    try:
        management.call_command("accession_availability", verbosity=0)
        return "success"
    except:
        print("error")
